...
const ShortenItem = ({ originalUrl, shortUrl, clickCount, createdDate }) => {
  ...
  const fetchMyShortUrl = async() => {
    setLoader(true);
    try{
      const { data } = await api.get(`/api/urls/analytics/${selectedUrl}?startDate=2024-12-31T00:00:00&endDate=2025-12-31T23:59:59` ,{
        headers:{
  ...
  return (
    <div className={`bg-slate-100 shadow-lg border border-dotted border-slate-500 px-6 sm:py-1 `}>
    ...
    <div className='flex items-center gap-2 font-semibold text-lg text-slate-800'>
     ...
     <div className="flex  flex-1  sm:justify-end items-center gap-4">
            ...
            >
                <div className="flex cursor-pointer gap-1 items-center bg-rose-500 py-2  font-semibold shadow-md shadow-slate-500 px-6 rounded-md text-white ">
            ...
            <div
              ...
              className='flex cursor-pointer gap-1 item-center bg-yellow-500 py-2 font-semibold shadow-md shadow-slate-500 px-6 rounded-md text-white'
            >
                       ...
                       <div className='flex flex-col items-center gap-1'>
                        {
                          <CircularProgress
                            height="100"
                            width="100"
                            color="linear-gradient(90deg,_#d53369_0%,_#daae51_100%)"
                            aria-label="circular-progress-loading"   // Changed from ariaLabel to aria-label (already fixed)
                            style={{}}                             // <<< Change wrapperStyle to style if this applies styles to the DOM element
                            // wrapperClass=""          // Confirm if this prop is supported by CircularProgress
                            // visible={true}
                            strokeWidth={2}
                            // animationDuration={1}
                          />
